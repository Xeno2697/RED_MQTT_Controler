...
class MQTTClientClass(mqtt.Client):
    ...
    #デバイス情報
    __DeviceInformationList = []    #リストのオブジェクトはDeviceInformation
    #スワーム情報
    ...
    def loop_forever(self, timeout=1, max_packets=1, retry_first_connection=False):
        time.sleep(1)
        return super().loop_forever(timeout, max_packets, retry_first_connection)
    #クライアントを起動する際の処理
    ...
    #FloorImage
    def DataProcessingforFloorImage(self, topic:str, payload):
        print("Topic:" + topic)
    ...
    #RobotStatus
    def DataProcessingforRobotStatus(self, topic:str, payload):
        print("Topic:" + topic)  
        ...
        for i in range(len(self.__DeviceInformationList)):
            if self.__DeviceInformationList[i].ID == search_id:
                ...
                break
        if b:
            self.__DeviceInformationList[len(self.__DeviceInformationList)] = DeviceInformation(int(datedict["Group"]),True,exp(),datedict["Battery"])      
        else:
