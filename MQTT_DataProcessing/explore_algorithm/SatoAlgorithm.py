...
class SatoAlgorithm:
    ...
    def main(self, floorcamera_status) -> None:
        # MQTTでパラメータを受信
        print('Get status')
    ...
    def __algorithm_exploration(self, floorcamera_status) -> None:
        ...
        try:
            ...
            distance, azimuth = self.__center_finder.getRTheta(self.__image_num, marker_color, height_correction_flag, self.gyroscope.angles)
            ...
            if self.__marker_num == 1:
                ...
                self.__center_finder.video_cap.frame(ceilImageName + '_ceil', self.__explore_parameters.shutter_speed)
                server.uploadImage(open('img/' + ceilImageName + '_ceil.png', 'rb').read(), ceilImageName + '_ceil.png', 'CeilImage')
            ...
            if floorcamera_status != "none":
                if self.__marker_num == 1:
                    self.__floorImage.UploadImage(self.__floorImage.takeImage(self.__step + 1, floorcamera_status), self.__current_distance_from_center[0], self.__step + 1, self.__azimuth_to_center[0])
                else: #マーカ2つの場合、次に向かうべきマーカからの距離・角度値を紐づけ(marker_num == 2)
                    self.__floorImage.UploadImage(self.__floorImage.takeImage(self.__step + 1, floorcamera_status), self.__current_distance_from_center[1], self.__step + 1, self.__azimuth_to_center[1])
        ...
        if self.__is_collision_for_goal == 1:
            ...
            self.__practical_transit_time = random_transit_time + 2.0
            self.__practical_rotate_angle = random_rotate_angle

            # サーバにデータをアップロード
            self.__upload_judge_result(False)
